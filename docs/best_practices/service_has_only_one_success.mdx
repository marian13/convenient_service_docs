...
You don't need to know the details of the implementation until you really need it.
...
*Service has only one `success`* actually means that a service calls `success` inside `result` only once.

But that `success` can still have multiple variations of data values, just like `display_name` in `BuildDisplayName`.
