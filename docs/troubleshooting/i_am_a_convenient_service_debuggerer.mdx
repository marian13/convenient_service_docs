...
For example, [HasResultSteps](/api/plugins/service/has_result_steps) has a middleware for the instance `result` method in [the Standard config](/api/configs/standard).
...
If your plugin enhances the instance `result` method as well, you may be affected by [HasResultSteps](/api/plugins/service/has_result_steps).
...
It works as a [proxy middleware](/glossary/proxy_middleware).

When a service has no steps, [HasResultSteps](/api/plugins/service/has_result_steps) just calls what is next in the middleware chain, by `chain.next(...)`.

Otherwise, it returns the last step copy.

Let's place, e.g. [HasResultParamsValidations::UsingActiveModelValidations](/api/plugins/service/has_result_params_validations/using_active_model_validations) plugin after [HasResultSteps](api/plugins/service/has_result_steps) in your [application service config](basics/tldr).
...
This way [HasResultParamsValidations::UsingActiveModelValidations](/api/plugins/service/has_result_params_validations/using_active_model_validations) is never called for services with steps.

To fix that, locate it before [HasResultSteps](/api/plugins/service/has_result_steps) like so:
