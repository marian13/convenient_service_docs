...
As the next step, we are going to require a [spec_helper](https://medium.com/@brett_codes/rspecs-spec-helper-vs-rails-helper-explained-2337282d14ce) and add a top-level [describe](https://rspec.info/features/3-12/rspec-core/example-groups/basic-structure) with the service class into it.
...
Then, we introduce a so-called [example group](https://rspec.info/documentation/3.0/rspec-core/RSpec/Core/ExampleGroup.html#context-class_method) for the class methods.
...
Coming back to the `AssertFileExists`, most of the time we interact with it using its `result` class method, which is why we add a corresponding [context](https://www.betterspecs.org/#describe) for it.
...
Also [Convenient Service](/) provides some ready-to-use custom [RSpec matchers](https://rspec.info/features/3-12/rspec-expectations/built-in-matchers) to verify service results.
...
The enclosing boilerplate blocks like `example_group "class methods"`, `describe ".result"`, <code>context "when \`Service\` is successful"</code> are straightforward.
