...
## What is the roption of a service with steps of `step`?

Check the following service that accepts two the following service.
...
As you can see, the corresponding methods' return values are passed as `:first_kwarg` and `:second_kwarg` to the `NestedService` step when we call the organizer.
